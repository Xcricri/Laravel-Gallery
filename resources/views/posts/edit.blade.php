                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form action="{{ route('posts.update', $post->id) }}" method="post" class="mt-6 space-y-6" >
                        ...
                        <div>
                            ...
                            <x-text-input id="title" class="mt-1 block w-full" name="title" :value="old('title', $post->title)"/>
                        ...
                        <div>
                            ...
                            <select name="category" id="category" class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm mt-1">
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('category') == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                            <x-input-error :messages="$errors->addPost->get('category')" class="mt-2"/>
                        </div>
                        {{-- div for status --}}
                        <div>
                            <div class="flex">
                            ...
                            <select name="status" id="status" class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm mt-1">
                                @php
                                    $currentStatus = old('status') ?? $post->status
                                @endphp
